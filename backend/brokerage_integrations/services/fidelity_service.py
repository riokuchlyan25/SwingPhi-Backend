...
class FidelityService(BaseBrokerageService):
    """Fidelity brokerage integration service"""
    ...
    def authenticate(self) -> bool:
        ...
        try:
            if self.api_key and self.secret_key:
                # Fidelity uses OAuth 2.0 with client credentials
                auth_data = {
            ...
            logger.error(f"Fidelity authentication failed: {e}")
            return False
    ...
    def get_portfolio(self) -> List[Dict[str, Any]]:
        """Get Fidelity portfolio positions"""
        try:
            if not self.authenticate():
                return []
    ...
    def get_transactions(self, start_date: datetime = None, end_date: datetime = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting Fidelity transactions: {e}")
            return []
    
    def get_balance(self) -> Dict[str, Any]:
        """Get Fidelity account balance"""
        try:
            if not self.authenticate():
                return self._format_error("Authentication failed")
    ...
    def _map_transaction_type(self, fidelity_type: str) -> str:
        ...
        return type_mapping.get(fidelity_type, 'transfer') 