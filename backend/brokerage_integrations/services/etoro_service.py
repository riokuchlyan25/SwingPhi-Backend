...
class EToroService(BaseBrokerageService):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.error(f"eToro authentication failed: {e}")
            return False
    
    def get_account_info(self) -> Dict[str, Any]:
        """Get eToro account information"""
        try:
            response = self._make_request('GET', f"{self.base_url}/user/profile")
            data = response.json()
            ...
            return self._format_response({
                ...
                'account_name': data.get('name', 'eToro Account'),
                'account_type': data.get('account_type'),
                'currency': data.get('currency', 'USD')
            })
        except Exception as e:
            logger.error(f"Error getting eToro account info: {e}")
            return self._format_error(str(e))
    
    def get_portfolio(self) -> List[Dict[str, Any]]:
        """Get eToro portfolio positions"""
        try:
            ...
            for position in data.get('positions', []):
                portfolio.append({
                    ...
                    'average_price': self._parse_decimal(position.get('open_price')),
    ...
    def get_transactions(self, start_date: datetime = None, end_date: datetime = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting eToro transactions: {e}")
            return []
    
    def get_balance(self) -> Dict[str, Any]:
        """Get eToro account balance"""
        try:
            ...
            logger.error(f"Error getting eToro balance: {e}")
            return self._format_error(str(e))
    
    def _map_transaction_type(self, etoro_type: str) -> str:
        """Map eToro transaction types to standard types"""
        type_mapping = {
            'BUY': 'buy',
            'SELL': 'sell',
            'DIVIDEND': 'dividend',
        ...
        return type_mapping.get(etoro_type, 'other')
    
    def get_brokerage_link(self) -> str:
        """Get eToro brokerage login link"""
        return "https://www.etoro.com/login" 